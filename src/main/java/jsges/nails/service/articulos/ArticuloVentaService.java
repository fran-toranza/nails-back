...
public class ArticuloVentaService implements IArticuloVentaService {
    ...
    public ResponseEntity<ArticuloVentaDTO> guardar(ArticuloVentaDTO model) {
        Linea linea = lineaService.getLineaEntityById(model.getLinea());
        if (linea == null) {
    ...
    public ResponseEntity<ArticuloVentaDTO> actualizar(ArticuloVentaDTO model, Integer id) {
        ...
        articuloExistente.setDenominacion(model.getDenominacion());
        articuloExistente.setCodigo(model.getCodigo());
        articuloExistente.setEstado(model.getEstado());
        articuloExistente.setDetalle(model.getDetalle());
        articuloExistente.setLinea(linea);

        return ResponseEntity.ok(ArticuloMapper.toDTO(modelRepository.save(articuloExistente)));
    }
}
